...
I chose to use a generalized linear mixed model, fitting the data to a Poisson distribution, as the response variable is discrete and all values are positive. Gamma would be a better fit, but it is not possible to use zero values with it and zeros make up a significant proportion of the observations. The model is as follows:
