...
I chose to use a generalized linear mixed model, fitting the data to a Poisson distribution, as it is not possible to use zero values with the gamma distribution and zeros make up a significant proportion of the observations. The model is as follows:

```{r model1, echo=T}
mdl1 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|status)+(1|reg/popu/gen)+(1|rack),
              family= poisson)
```
...
```{r testmod, echo=T}
mdl2 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|status)+(1|reg/popu/gen),
              family= poisson)  # rack removed
mdl3 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|reg/popu/gen)+(1|rack),
              family= poisson)  # status removed
mdl4 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|status)+(1|reg/popu)+(1|rack),
              family= poisson)  # gen removed
mdl5 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|status)+(1|popu/gen)+(1|rack),
              family= poisson)  # reg removed
mdl6 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|status)+(1|reg/gen)+(1|rack),
...
```{r testmod2, echo=T}
mdl5.1 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|popu/gen)+(1|rack),
              family= poisson)  # reg  and status removed
mdl5.2 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|status)+(1|gen)+(1|rack),
              family= poisson)  # reg  and popu removed
mdl5.3 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|status)+(1|popu)+(1|rack),
              family= poisson)  # reg and gen removed
mdl5.4 <- glmer(data = ara,formula = total.fruits+1 ~ nutrient*amd+(1|status)+(1|popu/gen),
...
According to the model selected, adding nutrients has a positive effect on fertility (p = 2e-16), whereas clipping plants has a negative effect on fertility (p = 2e-16). Looking at estimates reveals that the positive effect of nutrients is `r round(1.37329/0.42453,2)` times stronger than the negative effect of clipping the plant. The interaction between added nutrients and unclipped is negative (p = 2e-16), meaning that adding nutrients to unclipped plants increases fertility less than when added to clipped plants. 
