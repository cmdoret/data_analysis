...
In order to evaluate the effect of the different factors on the newborn weight, we can try to perform a linear model.



From the qqplot (Figure 2), the residuals of the model do not seem to deviate too much from the normality. The residuals vs fitted plot does not show a significant pattern or particular signs of heteroscedasticity. 
...
summary(lm(bwt ~ ht+smoke+lwt+ptd+age,data=birth_weight))
...
```{r dataviz,echo=F, fig.height= 8, fig.width=8, fig.cap="Visualization of the different variables against birth weight"}
